...
public class EvaluationRunApplicationService {
  ...
  public EvaluationRunApplicationService(
      EvaluationRunMapper runs,
      QualityGateMapper gates,
  ...
  public EvaluationRunResponse get(long userId, long runId) {
    ...
    if (run.getAnalysisTaskId() != null) {
      ...
      if (task.status() == AnalysisTaskStatus.SUCCESS && task.analysisResultId() != null)
        finalizeSuccess(run, task.analysisResultId());
      else if (task.status() == AnalysisTaskStatus.FAILED)
        finalizeFailure(run, task.safeErrorMessage());
    }
  ...
  private void finalizeSuccess(EvaluationRunDO run, long resultId) {
    ...
    JsonNode root;
    try {
      root = json.readTree(protocol.getRulesJson());
    } catch (Exception ex) {
      finalizeFailure(run, "Invalid protocol rules");
      return;
    }
    JsonNode metricRules = root.path("metrics");
    if (!metricRules.isArray() || metricRules.isEmpty()) {
      finalizeFailure(run, "Protocol has no metric rules");
      return;
    }
    boolean all = true;
    var metricJson = json.createObjectNode();
    for (JsonNode rule : metricRules) {
      String code = rule.path("code").asText("").toUpperCase();
      String comparison = rule.path("comparison").asText("LTE").toUpperCase();
      double threshold = rule.path("threshold").asDouble(Double.NaN);
      Double actual = metric(result, code);
      if (actual == null || !Double.isFinite(threshold)) {
        all = false;
        continue;
      }
      boolean passed = compare(actual, threshold, comparison);
      all &= passed;
      metricJson.put(code, actual);
      QualityGateDO gate = new QualityGateDO();
      gate.setEvaluationRunId(run.getId());
      gate.setMetricCode(code);
      gate.setActualValue(actual);
      gate.setThresholdValue(threshold);
      gate.setComparison(comparison);
      gate.setPassed(passed ? 1 : 0);
      gate.setDetail(passed ? "metric passed" : "metric exceeded protocol threshold");
      gates.insert(gate);
    }
    runs.finish(
        run.getId(), resultId, "SUCCESS", all ? "PASS" : "FAIL", metricJson.toString(), null);
  }

  private void finalizeFailure(EvaluationRunDO run, String message) {
    runs.finish(
        run.getId(), null, "FAILED", "FAIL", "{}", message == null ? "Evaluation failed" : message);
  }
  ...
  private EvaluationRunResponse response(EvaluationRunDO x) {
    ...
    if (x.getId() != null)
      for (var g : gates.selectByRunId(x.getId()))
        list.add(
            new QualityGateResponse(
                g.getMetricCode(),
                g.getActualValue(),
                g.getThresholdValue(),
                g.getComparison(),
                g.getPassed() != null && g.getPassed() == 1,
                g.getDetail()));
    return new EvaluationRunResponse(
        x.getId(),
        x.getSubmissionId(),
        x.getAnalysisTaskId(),
        x.getAnalysisResultId(),
        x.getStatus(),
        x.getGateStatus(),
        x.getMetricsJson(),
        x.getFailureMessage(),
  ...
  private Double metric(AnalysisResultDO r, String code) {
    return switch (code) {
      case "RMSE" -> r.getRmse();
      case "MEAN_ERROR" -> r.getMeanError();
      case "MIN_ERROR" -> r.getMinError();
      case "MAX_ERROR" -> r.getMaxError();
      case "STANDARD_DEVIATION" -> r.getStandardDeviation();
      case "ABNORMAL_RATIO" -> r.getAbnormalRatio();
      case "POINT_COUNT" -> r.getPointCount() == null ? null : r.getPointCount().doubleValue();
      default -> null;
    };
  }

  private boolean compare(double actual, double threshold, String op) {
    return switch (op) {
  ...
  private double number(String value, String key, double fallback) {
    try {
      return json.readTree(value).path(key).asDouble(fallback);
    } catch (Exception ex) {
      return fallback;
    }
  }

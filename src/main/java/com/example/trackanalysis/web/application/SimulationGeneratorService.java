...
public class SimulationGeneratorService {
  ...
  public SimulationGeneratorService(DatasetApplicationService datasets, TrackFileApplicationService files, AnalysisApplicationService analyses) {
  ...
  private String csv(SimulationRequest r) {
    ...
    for (int target = 0; target < r.targetCount(); target++) {
      for (int i = 0; i < r.points(); i++) {
        ...
        out.append(format(t)).append(',').append(format(truth[0])).append(',').append(format(truth[1])).append(',').append(format(truth[2])).append(',').append(format(x)).append(',').append(format(y)).append(',').append(format(z)).append('\n');
      }
    }
    return out.toString();
  }
  ...
  private double[] position(String model, double t, int target) {
    ...
    return switch (model.toUpperCase(Locale.ROOT)) {
      case "CONSTANT_ACCELERATION" -> new double[] {offset + 0.5 * 0.08 * t * t, 0.4 * t, Math.sin(t / 8.0)};
      case "CIRCULAR" -> new double[] {offset + 30 * Math.cos(t / 12.0), 30 * Math.sin(t / 12.0), Math.sin(t / 6.0)};
      default -> new double[] {offset + 1.2 * t, 0.4 * t, Math.sin(t / 8.0)};
    };
  }

  private double gaussian(Random random, double standardDeviation) { return standardDeviation == 0 ? 0 : random.nextGaussian() * standardDeviation; }
  private String format(double value) { return String.format(Locale.ROOT, "%.6f", value); }

  public record SimulationRequest(String motionModel, int dimensions, double noise, int targetCount, int points, double timeStep, long seed, double abnormalThreshold, String description) {
    public void validate() {
      if (!motionModel.equals("CONSTANT_VELOCITY") && !motionModel.equals("CONSTANT_ACCELERATION") && !motionModel.equals("CIRCULAR")) throw new IllegalArgumentException("不支持的运动模型");
      if (dimensions != 2 && dimensions != 3) throw new IllegalArgumentException("维度只能是 2 或 3");
      if (!Double.isFinite(noise) || noise < 0 || noise > 100) throw new IllegalArgumentException("噪声必须在 0 到 100 之间");
      if (targetCount < 1 || targetCount > 10 || points < 10 || points > 10000) throw new IllegalArgumentException("目标数或点数超出范围");
      if (!Double.isFinite(timeStep) || timeStep <= 0 || timeStep > 100) throw new IllegalArgumentException("采样间隔无效");
      if (!Double.isFinite(abnormalThreshold) || abnormalThreshold < 0 || abnormalThreshold > 1000) throw new IllegalArgumentException("异常阈值无效");
    }
  }

  private record GeneratedCsvFile(byte[] bytes) implements MultipartFile {
    public String getName() { return "file"; }
    public String getOriginalFilename() { return "simulation.csv"; }

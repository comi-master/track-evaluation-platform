...
# Milestone 6 acceptance scope

The final suite covers title validation, latest-per-file selection, deterministic/tied summaries, HTML escaping, owner-scoped 404 behavior, history pagination, content/download headers, V9 constraints and migration. Docker acceptance additionally checks non-root execution, health, Swagger, the business flow, log secrecy and exact cleanup without deleting named volumes. Test counts and measured outcomes are recorded only after the final commands run.

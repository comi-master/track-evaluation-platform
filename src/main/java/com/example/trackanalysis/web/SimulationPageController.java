...
public class SimulationPageController {
  ...
  public SimulationPageController(
      WebIdentityService identities, SimulationGeneratorService generator) {
  ...
  public String generate(
      Principal principal,
      @RequestParam String motionModel,
      @RequestParam int dimensions,
      @RequestParam double noise,
      @RequestParam int targetCount,
      @RequestParam int points,
      @RequestParam double timeStep,
      @RequestParam(defaultValue = "20260817") long seed,
      @RequestParam(defaultValue = "1.0") double abnormalThreshold,
      @RequestParam(required = false) String description,
      RedirectAttributes flash) {
    long id =
        generator.generate(
            identities.requireActive(principal.getName()).id(),

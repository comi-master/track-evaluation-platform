...
# Report API (milestone 6)

All report endpoints require Bearer JWT. `POST /api/v1/datasets/{datasetId}/reports` accepts `{"title":"..."}` (trimmed length 1–200) and returns 201 with metadata. It returns 404 for an absent/foreign dataset and 409 when no analyzed file exists. `GET /api/v1/reports/{reportId}` returns metadata only. `GET /api/v1/datasets/{datasetId}/reports?page=1&size=20` is capped at 100 and ordered by `created_at DESC,id DESC`. `GET /api/v1/reports/{reportId}/content` returns `text/html;charset=UTF-8`; `/download` additionally uses a safe attachment filename derived only from the numeric report ID. Foreign and absent reports both return 404.

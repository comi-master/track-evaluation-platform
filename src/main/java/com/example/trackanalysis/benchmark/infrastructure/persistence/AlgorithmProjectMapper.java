...
public interface AlgorithmProjectMapper extends BaseMapper<AlgorithmProjectDO> {
  @Select(
      "SELECT * FROM algorithm_project WHERE id=#{id} AND owner_user_id=#{userId} AND"
          + " status='ACTIVE' LIMIT 1")
  AlgorithmProjectDO selectOwnedById(@Param("id") long id, @Param("userId") long userId);

  @Select(
      "SELECT * FROM algorithm_project WHERE owner_user_id=#{userId} AND status='ACTIVE' ORDER BY"

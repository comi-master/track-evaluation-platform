...
public class EvaluationRunController {
  private final com.example.trackanalysis.evaluation.application.EvaluationRunApplicationService
      service;

  public EvaluationRunController(
      com.example.trackanalysis.evaluation.application.EvaluationRunApplicationService service) {
    this.service = service;
  }

  @PostMapping("/algorithm-submissions/{submissionId}/evaluate")
  public Result<EvaluationRunResponse> start(
      @AuthenticationPrincipal AuthenticatedUser user,
      @PathVariable @Min(1) long submissionId,
      HttpServletRequest request) {
    return Result.success(
        service.start(user.id(), submissionId), RequestIdFilter.requestId(request));
  }

  @GetMapping("/evaluations/{runId}")
  public Result<EvaluationRunResponse> get(
      @AuthenticationPrincipal AuthenticatedUser user,
      @PathVariable @Min(1) long runId,
      HttpServletRequest request) {
    return Result.success(service.get(user.id(), runId), RequestIdFilter.requestId(request));
  }

  @GetMapping("/evaluations/{runId}/gate")
  public Result<CiGateResponse> gate(
      @AuthenticationPrincipal AuthenticatedUser user,
      @PathVariable @Min(1) long runId,
      HttpServletRequest request) {
